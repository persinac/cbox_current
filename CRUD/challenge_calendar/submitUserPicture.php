...
if(isset($_FILES['upload'])) {
	...
	$storagePath = '/var/www/images/challenge_calendar/profiles/'.$user_id.'/';
    ...
    if (!$fileName) {
		...
		{
			//echo "Uploaded!";
			$stmt = $mysqli->prepare("update user_pub_info set picture_url=? where user_id=?");
			$stmt->bind_param( 'si', substr($completePath, 8), $user_id);
			...
			if($result = $stmt->execute()) {
				...
				$testDB->SetUpdateProfile("updated their profile picture");
				$testDB->SubmitToActivityLog($testDB->GetUpdateProfile(), 1);
